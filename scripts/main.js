//This is the set up JS file

function resizeCanvas() {
    if (!window.canvas) return;
    window.canvas.width = window.innerWidth;
    window.canvas.height = window.innerHeight;//global
}

window.addEventListener("resize", resizeCanvas);
window.addEventListener("DOMContentLoaded", () => {
    const canvas = document.querySelector("#art-canvas");

    window.canvas = canvas;//selecting the 'ID' from the styling file.
    window.ctx = canvas.getContext("2d");// make the canvas global

    resizeCanvas();

    initShark(canvas, ctx); // Calling the Shark JS functions
    
    spawnParticles();

    animate();
});



function animate() { //creating the animate function for my canvas
    requestAnimationFrame(animate);
    
    requestAnimationFrame(animate);
    if (!window.canvas || !window.ctx) return; //good practice, safety for animation loop.

    const w = canvas.width;
    const h = canvas.height;
    //ctx stands for 'context', representing a paintbrush. Everything visually gets drawn through ctx in JS.
    ctx.clearRect(0,0,w,h);
    
    //ocean layers
    ctx.fillStyle = "#0b2238";
    ctx.fillRect(0, h*0.3, w, h*0.4);
    ctx.fillStyle = "#062645";
    ctx.fillRect(0, h*0, w, h*0.3);
    //Overlapping layer that covers all, we can manipulate this later and create emotions, but leave transparent at first.
    ctx.fillStyle = "rgba(0,0,0,0.2)";
    ctx.fillRect(0, 0, w, h);

    if (window.shark && shark.naturalWidth) { //checking the states and allowing movements.
        ctx.drawImage(
            window.shark, 
            window.sharkState.x, 
            window.sharkState.y, 
            window.sharkState.w, 
            window.sharkState.h); // using the global structure with '.window'.
            
    }

    // calling particle functions in the other JS file for file structuring purposes.
    if (typeof animateparts === "function") {
        animateparts();
    }//ai assisted me here, telling me that if 'particles fail to initialise, the loop will break'.
    
    
}